...
public class ElectionManager {
	 ...
	 * maintaining the candidate list in alphabetical order by NAME, and returns the new total number of candidates in the array.
	 * Does NOT add the candidate if another candidate with the same name and party affiliation is already present in the list, 
	 * or if the provided vote count is a negative value, or if the input array has no room to add another candidate.
	 * 
	 * @param candidates A two-dimensional oversize array containing the current
	 * list of candidates as [name, party, numVotes]. This input value is assumed to
	 * conform to the standards of oversize arrays, and is assumed to be in alphabetical
	...
	{
		if (ElectionManager.containsCandidate(candidates, numCandidates, name, party) || numVotes < 0 || candidates.length == numCandidates)
		{
			return numCandidates;
		}
		int indexOfNew = numCandidates;
		String[][] newCandidates = new String [candidates.length][3];
		...
		for (int firstPart = 0; firstPart < indexOfNew; firstPart++)
		{
			newCandidates[firstPart] = candidates[firstPart];
		}
		newCandidates[indexOfNew][0] = name;
	...
	/**
	 * Finds the candidate with the majority (that is, >50%) of total votes cast. If no candidate in the list has 
	 * received more than 50% of the total votes, returns the string "CONTINGENT". This method will NOT be tested with a numCandidates of 0. 
	 * 
	 * @param candidates A two-dimensional oversize array containing the current
	 * list of candidates as [name, party, numVotes]. This input value is assumed to
	 * conform to the standards of oversize arrays, and is assumed to be in alphabetical
	 ...
	 * @param numCandidates The current size of the candidates oversize array at the time of input.
	 * This value is assumed to be accurate.
	 * @return A string containing the "name (party) - votePct%" values of the candidate receiving a majority of the votes,
	 * or the string "CONTINGENT" if no single candidate has received more than half of the votes.
	...
	/**
	 * Finds the candidate with the smallest number of votes cast. If there are multiple candidates with the same smallest number of votes, 
	 * this method returns the one whose name is closest to the beginning of the alphabet (smallest index). 
	 * If there are fewer than two candidates in the election, this method returns the string "UNCONTESTED".
	 * 
	 * @param candidates A two-dimensional oversize array containing the current
	 * list of candidates as [name, party, numVotes]. This input value is assumed to
	 * conform to the standards of oversize arrays, and is assumed to be in alphabetical
	 ...
	 * @param numCandidates The current size of the candidates oversize array at the time of input.
	 * This value is assumed to be accurate.
	 * @return A string containing the "name (party) - numVotes" values of the lowest-index candidate receiving the smallest number of votes,
	 * or "UNCONTESTED" if there are one or zero candidates in the list.
